document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch('/api/users/current-user')
        ...
        .then(data => {
            ...
            loadEndedSessions(); // Загружаем завершенные сессии после получения текущего пользователя
        })
        .catch(error => {
    ...
    function loadActiveSessions() {
        fetch('/api/sessions/active')
            ...
            .then(data => {
                ...
                data.forEach(session => {
                    ...
                    sessionElement.innerHTML = `
                        ...
                        <div class="session-details">
                            ...
                            ${currentUser && (currentUser.username === session.creator || session.joinedUsers.includes(currentUser.username)) ? `<button class="enter-button" data-session-id="${session.id}">Enter Session</button>` : ''}
                ...
                // Обработчики кнопок входа в сессию
